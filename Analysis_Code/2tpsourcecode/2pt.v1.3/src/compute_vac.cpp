...
{ 
	...
	cout<<"vactatmpassed "<<vactatmpassed<<" natom "<<natom<<" npass "<<vacnpass<<endl;
...
void ComputeVAC::calc_thermo_vals(ostream *outf, MODEL *model) 
{
	...
	for(i=0;i<ngrp-1;i++) {
		...
		if(model->grp[i].vol>0.0) vacV[i] = model->grp[i].vol;
	}
	...
	cout<<" Avg Thermo Vals:"<<endl;
	sprintf(null,"  %-12s : %.3f %s\n  %-12s : %.3f +/- %.3f %s\n  %-12s : %.3f %s\n  %-12s : %.3f %s",
	"Temperature",trjT,"(K)","Energy",vacE[ngrp-1][vactype-1]*4.184,model->grp[ngrp-1].eng.std*scale_f*4.184,"(kJ/mol)","Pressure",vacP,"(GPa)","Volume",vacV[ngrp-1],"(A^3)");
	*outf<<null<<endl;
	cout<<null<<endl;
	if(ngrp > 1) {
		...
		*outf<<null; cout<<null;
		for(i=0;i<ngrp-1;i++) {
			sprintf(null," %.3f +/- %.3f",vacE[i][vactype-1]*4.184,model->grp[i].eng.std*scale_f*4.184); *outf<<null; cout<<null;
		}
		*outf<<" (kJ/mol)"<<endl; cout<<" (kJ/mol)"<<endl;
	}
}
...
{ 
	...
	for(j=0;j<ngrp;j++) {
		...
		K2pt[0][j]=(pwr[0][j][0]/vlight*1E-2)/ngmol*sqrt(PI*Na*kb*vacT[j][vtrans]/(gmass*1e-3/ngmol))*2.0/9.0*pow(ngmol/gvol,1.0/3.0)*1E10*pow(6.0/PI,2.0/3.0); //translation
		f2pt[0][j]=search2PT(K2pt[0][j]);
		ry[0]=pow(f2pt[0][j]/K2pt[0][j],1.5);
		hs_sigma[0]=pow(ry[0]*6.0/PI/ngmol*gvol,1.0/3.0);
		...
		if(rT[0]>0.0) {
			K2pt[1][j]=(pwr[1][j][0]/vlight*1E-2)/ngmol*sqrt(PI*Na*kb*vacT[j][vangul]/(gmass*1e-3/ngmol))*2.0/9.0*pow(ngmol/gvol,1.0/3.0)*1E10*pow(6.0/PI,2.0/3.0); //rotation
		...
		for(i=0;i<nused;i++) {
			twoPT(&tmpg,&tmps,pwr[0][j][0],pwr[0][j][i],pwrfreq*i,ngmol,f2pt[0][j]);
...
{ 
	...
	*outf<<endl; out3n<<endl;
	...
	for(j=0;j<ngrp;j++) {	
		if(show2pt) { 
			sprintf(null," %4s[G%03d] %4s[G%03d] %4s[G%03d] %4s[G%03d]","Tgas",j+1,"Tsol",j+1,"Rgas",j+1,"Rsol",j+1); 
	...
	*outf<<endl; outthermo<<endl;
	...
	if(cmol) {
		...
		for(j=0;j<ngrp;j++) {
			...
			k=0;
			if(show2pt) {
				sprintf(null," %10d %10d %10d %10d",i,i,i,i); *outf<<null; outthermo<<null;
			}
			for(;k<vactype;k++) {
				sprintf(null," %10d",i); 
				*outf<<null; outthermo<<null;
	...
	*outf<<endl; outthermo<<endl;
	...
	for(j=0;j<ngrp;j++) {
		k=0;
		if(show2pt) {
			sprintf(null," %10.2f %10.2f %10.2f %10.2f",vacT[j][k],vacT[j][k],vacT[j][k],vacT[j][k]); 
			*outf<<null; outthermo<<null;
		}
		for(;k<vactype;k++) {
	...
	*outf<<endl; outthermo<<endl;
	...
	for(j=0;j<ngrp;j++)	{
		k=0;
		if(show2pt) {
			sprintf(null," %10.2f %10.2f %10.2f %10.2f",vacP,vacP,vacP,vacP); 
			*outf<<null; outthermo<<null;
		}
		for(;k<vactype;k++) {
	...
	*outf<<endl; outthermo<<endl;
	...
	for(j=0;j<ngrp;j++) {
		k=0;
		if(show2pt) {
			sprintf(null," %10.2f %10.2f %10.2f %10.2f",vacV[j],vacV[j],vacV[j],vacV[j]); 
			*outf<<null; outthermo<<null;
		}
		for(;k<vactype;k++) {
