...
void MODEL::element2prp(int do_mass)
{
	...
	for(i=0;i<natom;i++) {
		...
		if(strcmp(atom[i].name,"H")==0)	{
			atom[i].radius=1.20;
			atom[i].crad=0.37;
			if(do_mass) atom[i].mass=1.008;
		} else if(strcmp(atom[i].name,"He")==0) {
			atom[i].radius=1.40;
			atom[i].crad=0.00;
			if(do_mass) atom[i].mass=4.003;
		} else if(strcmp(atom[i].name,"C")==0)	{
			atom[i].radius=1.70;
			atom[i].crad=0.77;
			if(do_mass) atom[i].mass=12.01;
		} else if(strcmp(atom[i].name,"N")==0)	{
			atom[i].radius=1.55;
			atom[i].crad=0.74;
			if(do_mass) atom[i].mass=14.01;
		} else if(strcmp(atom[i].name,"O")==0)	{
			atom[i].radius=1.52;
			atom[i].crad=0.74;
			if(do_mass) atom[i].mass=16.00;
		} else if(strcmp(atom[i].name,"F")==0)	{
			atom[i].radius=1.47;
			atom[i].crad=0.72;
			if(do_mass) atom[i].mass=19.00;
		} else if(strcmp(atom[i].name,"Na")==0) {
			atom[i].radius=2.27;
			atom[i].crad=1.66;
			if(do_mass) atom[i].mass=22.99;
		} else if(strcmp(atom[i].name,"S")==0)	{
			atom[i].radius=1.80;
			atom[i].crad=1.04;
			if(do_mass) atom[i].mass=32.07;
		} else if(strcmp(atom[i].name,"Cl")==0) {
			atom[i].radius=1.75;
			atom[i].crad=0.99;
			if(do_mass) atom[i].mass=35.45;
		} else if(strcmp(atom[i].name,"Pt")==0) {
			atom[i].radius=1.75;
			atom[i].crad=1.21;
			if(do_mass) atom[i].mass=195.08;
		} else {
			atom[i].radius=0.9;
			atom[i].crad=0.10;
			if(do_mass) atom[i].mass=0.01;
		}
